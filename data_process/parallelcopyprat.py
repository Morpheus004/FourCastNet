...
#             # TODO: Removed MPI code from here , driver='mpio', comm=MPI.COMM_WORLD
...
def writetofile(src, dest, channel_idx, varslist, src_idx=0, frmt='nc'):
    if os.path.isfile(src):
        ...
        for variable_name in varslist:
            ...
            idx = 0
            while idx < Nimgtot:
                # Handle 4D and 3D source arrays
                if len(fsrc.shape) == 4:
                    ims = fsrc[idx:idx+batch, src_idx]
                else:
                    ims = fsrc[idx:idx+batch]
                ...
                # Write to specific channel in fields dataset
                fdest['fields'][idx:idx+ims.shape[0], channel_idx, :, :] = ims
